...
- input: a string
- output: output of the program (if any)

- goal: to implement a miniature stack-and-register-based programming language
- rules
  * register holds single value, where stack holds a list of values
  * every space delimited word is a command -> i need to parse input based on spaces
  * initialize register to 0
  * implement commands described
  * assume program inputs are always correct, no unknown commands

* algorithm:
  * initialize stack to an empty array
  * initialize register to 0
  * split input by spaces and assign to commands variable
  * iterate through commands array
    * execute the command -> manipulate register or stack
                          -> or print a value
